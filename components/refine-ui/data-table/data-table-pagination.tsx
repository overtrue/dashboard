...
export function DataTablePagination({
  ...
  const pageSizeOptions = useMemo(() => {
    const baseOptions = [15, 25, 30, 40, 50]
    const optionsSet = new Set(baseOptions)
    if (!optionsSet.has(pageSize)) optionsSet.add(pageSize)
  ...
  return (
    <div className={cn("flex", "items-center", "justify-between", "flex-wrap", "w-full", "gap-2")}>
      <div className={cn("flex-1", "text-sm", "text-muted-foreground", "whitespace-nowrap")}>
        {typeof total === "number" ? `${total} ${total === 1 ? "row" : "rows"}` : null}
      </div>
      <div className={cn("flex", "items-center", "flex-wrap", "gap-2")}>
        <div className={cn("flex", "items-center", "gap-2")}>
          ...
          <Select value={`${pageSize}`} onValueChange={(v) => setPageSize(Number(v))}>
            <SelectTrigger className={cn("h-8", "w-[70px]")} aria-label="Rows per page">
        ...
        <div className={cn("flex", "items-center", "flex-wrap", "gap-2")}>
          ...
          <div className={cn("flex", "items-center", "gap-2")}>
            ...
            >
              <RiArrowLeftDoubleLine aria-hidden />
            </Button>
            <Button
              variant="outline"
            ...
            >
              <RiArrowRightDoubleLine aria-hidden />
            </Button>
          </div>
        </div>
      </div>
